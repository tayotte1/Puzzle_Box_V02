// Initialize the servos to initial position
void initServo() {
  pinMode(PIN_SERVO_LID_PWR, OUTPUT);           // Initialize the lid servo power pin
  digitalWrite(PIN_SERVO_LID_PWR, LOW);         // Set LOW
  
  movePizzaServo(PIN_SERVO_PIZZA, POS_PIZZA_INIT);   // Initialize pizza servo to initial position
  delay(500);
  movePizzaServo(PIN_SERVO_PIZZA, POS_PIZZA_FINAL);   // Initialize pizza servo to initial position
  delay(500);
  movePizzaServo(PIN_SERVO_PIZZA, POS_PIZZA_INIT);   // Initialize pizza servo to initial position
}
...
** pos, integer of the position to move the servo to
*/
void movePizzaServo(int pin, int pos) {
  myServo.attach(pin);                    // attach it to myServo
  myServo.write(pos);                     // send it to proper position
  delay(100);                             // let it move
  myServo.detach();                       // detach it
}
