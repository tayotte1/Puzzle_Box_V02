...
** Parameters:
** val, integer of the value that it thinks it should write
*/
void lcdWriteNumber(int val) {
// if input less than 1 then roll counter to 12 because we want to only display 1-12 for months
  if(val < 0) {
    encoderValue = 99*4+2;            // encoder starts at -1 so need to add 2
  }
// if input greater than 12 then roll counter to 1 because we want to only display 1-12 for months
  if(val > 99) {
    encoderValue = 0*4+2;             // encoder starts at -1 so need to add 2
  }


  lcdNumber = encoderValue/4;         // set lcdMonth
  
  lcd.setCursor(14,1);                 // move cursor to month
...
// if going to display month < 10, add leading zero
  ...
  lcd.print(lcdNumber);               // print the month to the LCD
  lcd.setCursor(14,1);                 // move cursor back
  Serial.print("Number: ");           // debug printing
  Serial.println(lcdNumber);          // debug printing
}
