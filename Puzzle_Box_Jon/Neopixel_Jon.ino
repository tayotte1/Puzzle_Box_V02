...
void flashRainbow(int num) {
  for(int j=0; j<num; j++) {                                                        // Do this 'num' amount of times
    for(int i=0; i< pixels.numPixels(); i++) {                                      // For number of Neopixels
      pixels.setPixelColor(i, Wheel(((i * 256 / pixels.numPixels()) + j) & 255));   // Set pixel color to calculated rainbow color
    }
...
void rainbowCycle(int wait, int num) {
  ...
  for(j=0; j<256*num; j++) {                    // Cycles of all colors on wheel num amount of times
    for(i=0; i< pixels.numPixels(); i++) {      // Set each pixel
      pixels.setPixelColor(i, Wheel(((i * 256 / pixels.numPixels()) + j) & 255)); // Set pixel to calculated rainbow color
    }
...
// Copied from Neopixel library. Didn't end up using.
